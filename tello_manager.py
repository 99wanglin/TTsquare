...
class Tello_Manager:
    def __init__(self):
        ...
        # we invoked another instance of this thread to receive data for processing
    ...
    def send_command(self, command, ip):
        ...
        if multi_cmd_send_flag == True:      
            ...
            for num in range(1,5):                
                ...
                #sends the cmd to all the drones
            ...
            #sends the cmd to the single drone
        ...
        while not self.log[ip][-1].got_response():
            ...
            if ping_count == 5:
                if multi_cmd_send_flag == True:  
                    self.socket.sendto(cmd.encode('utf-8'), (ip, 8889))
                    print 'RESEND [Multi_Command]----Multi_Send----IP:%s----Command:   %s\n' % (ip, command[3:]) 
                else: 
                    self.socket.sendto(command.encode('utf-8'), (ip, 8889))
                    print 'RESEND [Single_Command]----Single_Send----IP:%s----Command:   %s\n' % (ip, command)
